...
⍝ ATTENTION: the table between the two markers is GENERATED by `Admin.UpdateCommandTable` as
⍝ part of `Make`. Do not edit it here - edit the `Desc:`, `Parse:` and `Project:` comments in
⍝ the user command functions themselves, and then run `Make`.\\
⍝ Note that the body of this function is injected into the user command script as well, where
⍝ it must run without APLGit2 being loaded at all: keep it free of any dependencies.
 r←⍬
 table←0 4⍴⊂''
...
⍝ >>>>> StartTableInject
     ...
     table⍪←'ShowLastVersionInIndex' '1 -project=' 1 'Returns the last version of an APL object or a file in the index'

...
For example, the Git command `switch` and `checkout` should not be performed within an APL session.  
...
1. Most online Git resources and examples focus on the Git Bash.
...
```
 ...
 ]Repack                 Optimize Git's internal storage, saving space & speeding things up
 ]ShowLastVersionIndex   Shows the last version of an APL object in the index (last commit)
 ]StashApply             Applies the last or the given stash and keep it in the stash cache

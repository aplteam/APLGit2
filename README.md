...
`APLGit2` includes a concise set of user commands and functions to help APL programmers manage Git projects effectively.

`APLGit2` runs on Windows, Linux and macOS.

## Philosophy

Experience shows that attempting to support all Git commands from within APL often leads to complications.  

For example, the Git commands `switch` and `checkout` should not be performed within an APL session.  
...
Commands like `Add -i`, which interact with the user, cannot be called at all from APL.

Therefore, `APLGit2` offers only a limited set of useful commands from within APL for project management. For anything else, we recommend using Git Bash. `APLGit2` simplifies this process by providing the command `]APLGit2.OpenGitShell`, which opens a Git Bash session in the current project (if only one is open) or in a specified/selected project.
...
For instance, the `Commit` API function solely executes `git commit`.  In contrast, the corresponding user command checks if the project has uncommitted changes and prompts the user to run `git add .` first. It also opens an editor for composing a commit message if the `-m=` option is omitted.
...
One function, `GetTagOfLatestRelease`, requires that your project be hosted on GitHub.  
...
`]APLGit2` cooperates with the project manager [acre](https://github.com/the-carlisle-group/acre-desktop) in the same way it cooperates with Cider, so everything said there applies to acre as well.
...
If you want the API to be available from the start, refer to the [Dyalog User Commands](https://aplwiki.com/wiki/Dyalog_User_Commands "Link to the APL wiki") article.
...
```
 ...
 ]Repack                 Optimise Git's internal storage, saving space & speeding things up
 ]ShowLastVersionIndex   Shows the last version of an APL object in the index (last commit)
 ]StashApply             Applies the last or the given stash and keeps it in the stash cache
 ]StashClear             Drops all saved stashes from the stash cache
 ]StashDrop              Drops a given stash (by default the latest): removes it from the stash cache
 ...
 ]StashPush              Saves modifications as a new stash entry & rolls them back
 ]StashShow              Show changes between stash and its original parent
